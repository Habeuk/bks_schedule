...
const r = { _eid: null, fromVueCal: null, toVueCal: null }, C = class {
  constructor(t) {
    V(this, "_vuecal"), this._vuecal = t;
  }
  _getEventStart(t) {
  ...
  cellDragDrop(t, e, l, i) {
    ...
    if (r.fromVueCal !== this._vuecal._.uid) {
      const { _eid: s, start: p, end: D, duration: g, ...w } = n;
      a = this._vuecal.utils.event.createAnEvent(l, g, { ...w, split: i });
    } else if (a = this._vuecal.view.events.find((s) => s._eid === r._eid), a || (a = this._vuecal.mutableEvents.find((s) => s._eid === r._eid), d = !!a), !a) {
      const s = n.endTimeMinutes - n.startTimeMinutes, { start: p, end: D, ...g } = n;
      a = this._vuecal.utils.event.createAnEvent(l, s, { ...g, split: i });
    }
    const { start: T, split: f } = a;
    ...
    const h = { event: this._vuecal.cleanupEvent(a), oldDate: T, newDate: a.start, ...(i || i === 0) && { oldSplit: f, newSplit: i }, originalEvent: this._vuecal.cleanupEvent(n), external: !r.fromVueCal };
  ...
  viewSelectorDragEnter(t, e, l) {
    t.currentTarget.contains(t.relatedTarget) || (l.highlightedControl = e, clearTimeout(o), o = setTimeout(() => {
      if (["previous", "next"].includes(e))
        ...
        let i;
        clearInterval(c), this._vuecal.view.id.includes("year") && (i = this._vuecal.enabledViews.filter((n) => !n.includes("year"))[0]), this._vuecal.switchView(i || this._vuecal.view.id, new Date((/* @__PURE__ */ new Date()).setHours(0, 0, 0, 0)), !0);
      } else
